    ...
    class FirebaseDao {
      ...
      Stream<List<Post>> readUserPosts(String userId) {
        return postsCollection.where('userID', isEqualTo: userId).snapshots().map((snapshot) {
    ...
    // Kullanıcı verilerini güncellemek için
      ...
      Future<List<Post>> getTwoPosts() async {
        try {
          final querySnapshot = await postsCollection.limit(3).get();
          return querySnapshot.docs.map((doc) {
            final data = doc.data() as Map<String, dynamic>;
            return Post.fromJson(data);
          }).toList();
